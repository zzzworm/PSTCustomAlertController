...
{
    PSTAlertAction *action = [[self class]allocWithZone:zone];
...
@property (nonatomic, strong) TYAlertController *customController;
...
- (instancetype)initWithTitle:(NSString *)title message:(NSString *)message preferredStyle:(PSTAlertControllerStyle)preferredStyle {
    if ((self = [super init])) {
        ...
        if (preferredStyle ==  PSTAlertControllerStyleCustomActionSheet) {
            TYAlertView *alertView = [TYAlertView alertViewWithTitle:title message:message];

            _customController = [TYAlertController alertControllerWithAlertView:alertView preferredStyle:TYAlertControllerStyleActionSheet];
            _customController.backgoundTapDismissEnable = YES;
        }
        else if (preferredStyle == PSTAlertControllerStyleCustomAlert){
            TYAlertView *alertView = [TYAlertView alertViewWithTitle:title message:message];
            
            _customController = [TYAlertController alertControllerWithAlertView:alertView preferredStyle:TYAlertControllerStyleAlert];
        }
        else{
...
- (void)setMessage:(NSString *)message {
    _message = [message copy];
    _customController.message = message;
...
- (void)addAction:(PSTAlertAction *)action {
    ...
    if(self.preferredStyle >= PSTAlertControllerStyleCustomActionSheet){
        [self.customController.alertView addAction:action];
    }
    else if ([self alertControllerAvailable]) {
        ...
        if (self.preferredStyle == PSTAlertControllerStyleActionSheet) {
            ...
            if (!action.enabled) {
                NSInteger buttonIndex = currentButtonIndex;
...
- (void)addTextFieldWithConfigurationHandler:(void (^)(UITextField *textField))configurationHandler {
    if(self.preferredStyle >= PSTAlertControllerStyleCustomActionSheet){
        [self.customController.alertView addTextFieldWithConfigurationHandler:configurationHandler];
    }
    else if ([self alertControllerAvailable]) {
...
- (NSArray *)textFields {
    if (self.preferredStyle >= PSTAlertControllerStyleCustomActionSheet) {
        return self.customController.textFields;
    }
    if ([self alertControllerAvailable]) {
...
- (BOOL)isVisible {
    if(self.preferredStyle >= PSTAlertControllerStyleCustomActionSheet){
        return self.customController.view.window !=nil;
    }
    else if ([self alertControllerAvailable]) {
...
- (void)showInController:(nullable UIViewController *)controller animated:(BOOL)animated completion:(void (^ __nullable)(void))completion
{
    [self showWithSender:nil controller:controller animated:animated completion:completion];
}
...
- (void)showWithSender:(id)sender arrowDirection:(UIPopoverArrowDirection)arrowDirection controller:(UIViewController *)controller animated:(BOOL)animated completion:(void (^)(void))completion {
    if(self.preferredStyle >= PSTAlertControllerStyleCustomActionSheet){
        ...
        TYAlertController *alertController = self.customController;
        ...
        __weak typeof (self) weakSelf = self;
        alertController.viewWillDisappearBlock = ^(UIView *alertView){
        ...
        alertController.viewDidDisappearBlock = ^(UIView *alertView){
            typeof (self) strongSelf = weakSelf;
...
- (void)dismissAnimated:(BOOL)animated completion:(void (^)(void))completion {
    if(self.preferredStyle >= PSTAlertControllerStyleCustomActionSheet){
        [self.customController dismissViewControllerAnimated:animated completion:completion];
    }
    else if ([self alertControllerAvailable]) {
...
- (void)setPreferredAction:(PSTAlertAction *)preferredAction {
    _preferredAction = preferredAction;
    if(self.preferredStyle >= PSTAlertControllerStyleCustomActionSheet){
        [self.customController.alertView setPreferredAction:preferredAction];
    }
    else {
        if(@available(iOS 9, *)) {
            NSInteger index = [self.actions indexOfObject:preferredAction];
            if (index < self.alertController.actions.count) {
                UIAlertAction *action = [self.alertController.actions objectAtIndex:index];
                [self.alertController setPreferredAction:action];
            }
        }
    }
}
...
+ (instancetype)customActionSheetWithTitle:(NSString *)title
{
    return [[self alloc] initWithTitle:title message:nil preferredStyle:PSTAlertControllerStyleCustomActionSheet];
}
...
+ (instancetype)presentDismissableAlertWithTitle:(NSString *)title message:(NSString *)message controller:(UIViewController *)controller {
    ...
    [alertController addAction:[PSTAlertAction actionWithTitle:NSLocalizedString(@"Dismiss",nil) style:PSTAlertActionStyleCancel handler:NULL]];
...
- (void)addCancelActionWithHandler:(void (^)(PSTAlertAction *action))handler {
    NSString *cancelTitle = self.preferredStyle == PSTAlertControllerStyleAlert|| self.preferredStyle == PSTAlertControllerStyleCustomAlert ? NSLocalizedString(@"Cancel", nil) : NSLocalizedString(@"CANCEL", nil);
    [self addAction:[PSTAlertAction actionWithTitle:cancelTitle style:PSTAlertActionStyleCancel handler:handler?:^(PSTAlertAction * _Nonnull action) {
